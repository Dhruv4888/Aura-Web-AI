...
if text:
    ...
    with st.spinner("Gyan Setu is thinking..."):
        full_response = ""
        current_sentence = ""
        # UI Container for Gyan Setu's response
        container = st.empty()
        
        # --- SIR'S REQUIREMENT LOGIC (Sentence-by-Sentence) ---
        for chunk in aura.ask_stream(text, st.session_state.messages):
            full_response += chunk
            current_sentence += chunk
            
            # Rendering with cursor inside original container
            container.markdown(f'<div class="chat-container"><b>Gyan Setu:</b> {full_response}▌</div>', unsafe_allow_html=True)
            
            # Sentence detection for immediate voice
            if any(punc in chunk for punc in [".", "!", "?", "\n"]):
                if len(current_sentence.strip()) > 15: # Thoda bada sentence hone par hi bolega
