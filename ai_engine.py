...
class AuraAssistant:
    ...
    def is_hindi(self, text):
        # Hindi script detection logic
        return bool(re.search(r'[\u0900-\u097F]', text))

    def clean_text_for_speech(self, text):
        # Audio ke liye text ki safai
        text = re.sub(r'\[WIKI_SEARCH:.*?\]', '', text)
        text = text.replace("$", "").replace("#", "").replace("*", "").replace("`", "")
        text = re.sub(r'\\text\{.*?\}', '', text) 
        
        # Gender corrections teacher persona ke liye
    ...
    async def _generate_voice(self, text, filename):
        # Awaaz selection script ke basis par
    ...
    def get_audio_data(self, text):
        """Audio generate karke uska base64 format bhejta hai"""
